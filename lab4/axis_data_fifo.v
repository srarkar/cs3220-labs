...
// when FIFO is empty, the read pointer will only differ from the write pointer by the MSB
//wire full = (wr_ptr_gray_reg[ADDR_WIDTH - 1:0] == rd_ptr_gray_sync2_reg[ADDR_WIDTH - 1:0]) 
    // && (wr_ptr_gray_reg[ADDR_WIDTH] == rd_ptr_gray_sync2_reg[ADDR_WIDTH]);
...
// from readme: when FIFO is empty, the read pointer will be equal to the write pointer
...
always @* begin
    ...
    if (rd_axis_rdy) begin
        store_output = mem_read_data_valid_reg;
